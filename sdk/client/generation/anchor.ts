 ...
 * @throws Error if PDA derivation fails.
 */
export function getUserObligationAccount(user: PublicKey, pool: PublicKey): PublicKey {
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    throw new Error(`Failed to derive user obligation account: ${errorMessage}`);
  }
}
...
export function getCollateralVaultAccount(pool: PublicKey): PublicKey {
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    throw new Error(`Failed to derive collateral vault account: ${errorMessage}`);
  }
}
