...
mod circuits {
    ...
    // ============================================================
    // LEGACY Circuits (Backwards Compatibility)
    // These reveal amounts and should be deprecated after migration
    // ============================================================

    /// Output from deposit computation (LEGACY - reveals amount)
    ...
    /// Compute deposit: adds amount to user's deposit balance (LEGACY)
    #[instruction]
    ...
    /// Output from borrow computation (LEGACY - reveals amount)
    ...
    /// Output from withdraw computation (LEGACY - reveals amount)
    ...
    /// Output from repay computation (LEGACY - reveals amount)
    pub struct RepayOutput {
        pub new_state: UserState,
        pub repay_delta: u64,
    }

    /// Compute repay (LEGACY)
    #[instruction]
    pub fn compute_repay(
        ...
        let principal_payment = actual_repay - interest_payment;
        let new_borrow = current_state.borrow_amount - 
            principal_payment.min(current_state.borrow_amount);
    ...
    /// Output from liquidation computation (LEGACY - reveals amounts)
    ...
    /// Output from interest computation (LEGACY - reveals amount)
    pub struct InterestOutput {
        pub new_state: UserState,
        pub interest_accrued: u64,
    }

    /// Compute interest accrual (LEGACY)
    #[instruction]
