 ...
 */
export const toArciumX25519PublicKey = (bytes: Uint8Array): ArciumX25519PublicKey => {
  if (bytes.length !== 32) throw new Error(`Invalid length for ArciumX25519PublicKey: ${bytes.length}`);
 ...
 * @throws Error if length is not 32.
 */
export const toArciumX25519SecretKey = (bytes: Uint8Array): ArciumX25519SecretKey => {
  if (bytes.length !== 32) throw new Error(`Invalid length for ArciumX25519SecretKey: ${bytes.length}`);
...
export const toRescueCiphertext = (bytes: Uint8Array): RescueCiphertext => {
  if (bytes.length !== 32) throw new Error(`Invalid length for RescueCiphertext: ${bytes.length}`);
 ...
 */
export const toArciumX25519Nonce = (val: number | BN | Uint8Array): ArciumX25519Nonce => {
